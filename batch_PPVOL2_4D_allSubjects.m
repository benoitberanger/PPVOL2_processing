...
chemin={[pwd filesep 'PRISMA_PPVOL2_allSubjects']}
...
fanat = get_subdir_regex_files(anat,'^s_',1)
unzip_volume(fanat)
fanat = get_subdir_regex_files(anat,'^s_',1)
...
%smooth the data
ffonc = fo %ffonc = get_subdir_regex_files(dfonc,'^wutrf')
par.smooth = [6 6 6];
...
fons_localizer = {repmat(char(get_subdir_regex_files(onsetdir,'^loca',1)),[2 1])}
fons_localizer = repmat(fons_localizer,size(suj))
fons_meridian = get_subdir_regex_files(onsetdir,'^meridian',1);
fons_meridian = repmat(fons_meridian,size(suj))
